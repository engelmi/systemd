...
DEFINE_PRIVATE_HASH_OPS_WITH_VALUE_DESTRUCTOR(
                group_hash_ops, char, path_hash_func, path_compare, Group, group_free);
...
#define MAYBE_FORMAT_TIMESPAN(t, accuracy) maybe_format_timespan((char[BUFSIZE1]){}, BUFSIZE1, t, accuracy)
...
#define MAYBE_FORMAT_BYTES(is_valid, t) maybe_format_bytes((char[BUFSIZE2]){}, BUFSIZE2, is_valid, t)
...
static bool is_root_cgroup(const char *path) {
         ...
         * cgroup v2 as it carries only very few attributes in order not to export multiple truth about
         ...
         * effect (CLONE_NEWCGROUP). Since there's no nice way to distinguish a real cgroup root from a fake
         ...
         * However, that's mostly a theoretic use case, and if people actually try all they'll lose is
...
static int
                refresh_one(const char *controller,
                            const char *path,
                            Hashmap *a,
                            Hashmap *b,
                            unsigned iteration,
...
static void display(Hashmap *a) {
        ...
        for (unsigned j = 0; j < n; j++) {
                ...
                maxtpath = MAX(maxtpath, strlen(array[j]->path));
        }
        ...
        if (on_tty()) {
                ...
                printf("%s%s%-*s%s %s%7s%s %s%*s%s %s%8s%s %s%8s%s %s%8s%s%s\n",
                       ...
                       arg_count == COUNT_PIDS                                ? "Tasks" :
                                       arg_count == COUNT_USERSPACE_PROCESSES ? "Procs" :
                                                                                "Proc+",
...
static void sendit(CGTopAgent *agent) {
        Group *g;
        Group **array;
        unsigned n = 0;
        unsigned top_n = 10;
        ...
        array = newa(Group *, hashmap_size(agent->cg_b));

        HASHMAP_FOREACH(g, agent->cg_b)
                if (g->n_tasks_valid || g->cpu_valid || g->memory_valid || g->io_valid)
                        array[n++] = g;
        ...
        int r = sd_bus_message_new_signal(
                        agent->systemd_dbus, &sig, "/org/eclipse/bluechi", "org.eclipse.bluechi", "CGTop");
        if (r < 0) {
                printf("Failed to create CGTop signal: %s", strerror(-r));
                return;
        }

        r = sd_bus_message_open_container(sig, SD_BUS_TYPE_ARRAY, "(stdsss)");
        if (r < 0) {
                printf("Failed to open array of CGTop signal: %s", strerror(-r));
                return;
        }

        for (unsigned j = 0; j < top_n; j++) {
                g = array[j];

                const char *path = empty_to_root(g->path);
                uint64_t tasks = g->n_tasks_valid ? g->n_tasks : 0;
                double cpu = g->cpu_valid ? g->cpu_fraction * 100 : 0;
                _cleanup_free_ char *memory = NULL;
                asprintf(&memory, "%8s", MAYBE_FORMAT_BYTES(g->memory_valid, g->memory));
                if (memory == NULL) {
                        printf("Couldn't format memory, OOM");
                        return;
                }
                _cleanup_free_ char *io_input = NULL;
                asprintf(&io_input, "%8s", MAYBE_FORMAT_BYTES(g->io_valid, g->io_input_bps));
                if (io_input == NULL) {
                        printf("Couldn't format memory, OOM");
                        return;
                }
                _cleanup_free_ char *io_output = NULL;
                asprintf(&io_output, "%8s", MAYBE_FORMAT_BYTES(g->io_valid, g->io_output_bps));
                if (io_output == NULL) {
                        printf("Couldn't format io_output, OOM");
                        return;
                }

                r = sd_bus_message_open_container(sig, SD_BUS_TYPE_STRUCT, "stdsss");
                if (r < 0) {
                        printf("Failed to open array of CGTop signal: %s", strerror(-r));
                        return;
                }

                r = sd_bus_message_append(sig, "stdsss", path, tasks, cpu, memory, io_input, io_output);
                if (r < 0) {
                        printf("Failed to append data to CGTop signal: %s", strerror(-r));
                        return;
                }

                r = sd_bus_message_close_container(sig);
                if (r < 0) {
                        fprintf(stderr, "Failed closing CGTop array: %s\n", strerror(-r));
                        return;
                }
        }
        ...
        r = sd_bus_send(agent->systemd_dbus, sig, 0);
        if (r < 0) {
                printf("Failed to send signal to monitor owner: %s", strerror(-r));
        }
}
...
static int loop(CGTopAgent *agent) {

        int r = refresh(agent->root, agent->cg_a, agent->cg_b, agent->iteration++);
        if (r < 0)
                return log_error_errno(r, "Failed to refresh: %m");

        hashmap_clear(agent->cg_b);
        SWAP_TWO(agent->cg_a, agent->cg_b);
...
static int run(int argc, char *argv[]) {
        ...
        _cleanup_cgtop_agent_ CGTopAgent *agent = cgtop_agent_new(root);
        if (agent == NULL) {
                return -1;
        }
        cgtop_agent_setup_timer(agent);

        if (!sd_event_loop(agent->event)) {
                printf("Failed to start event loop");
                return -1;
        }
        return 0;
}
...
static const sd_bus_vtable vtable[] = { SD_BUS_VTABLE_START(0),
                                        SD_BUS_SIGNAL_WITH_NAMES("CGTop", "a(stdsss)", SD_BUS_PARAM(cgtop), 0),
                                        SD_BUS_VTABLE_END };
...
CGTopAgent *cgtop_agent_new(const char *root) {
        ...
        if (r < 0) {
                printf("Failed to connect to system bus: %s", strerror(-r));
                return NULL;
        }
        ...
        if (r < 0) {
                printf("Failed to create event loop: %s", strerror(-r));
                return NULL;
        }

        r = sd_bus_attach_event(bus, event, SD_EVENT_PRIORITY_NORMAL);
        if (r < 0) {
                printf("Failed to attach bus to event: %s", strerror(-r));
                return NULL;
        }

        printf("Connected to system bus");

        _cleanup_free_ char *service_name = strdup("org.eclipse.bluechi");
        if (service_name == NULL) {
                printf("Failed to allocate memory, OOM");
                return NULL;
        }
        _cleanup_free_ char *rootcp = strdup(root);
        if (rootcp == NULL) {
                printf("Failed to allocate memory, OOM");
                return NULL;
        }

        r = sd_bus_request_name(bus, "org.eclipse.bluechi", SD_BUS_NAME_REPLACE_EXISTING);
        if (r < 0) {
                printf("Failed to acquire service name on api dbus: %s", strerror(-r));
                return NULL;
        }

        _cleanup_cgtop_agent_ CGTopAgent *agent = malloc0(sizeof(CGTopAgent));
        agent->ref_count = 1;

        r = sd_bus_add_object_vtable(
                        bus, &agent->cgtop_slot, "/org/eclipse/bluechi", "org.eclipse.bluechi", vtable, agent);
        if (r < 0) {
                printf("Failed to add manager vtable: %s", strerror(-r));
                return NULL;
        }

        agent->cg_a = hashmap_new(&group_hash_ops);
        agent->cg_b = hashmap_new(&group_hash_ops);
        if (!agent->cg_a || !agent->cg_b) {
                log_oom();
                return NULL;
        }
        agent->api_bus_service_name = steal_pointer(&service_name);
        agent->root = steal_pointer(&rootcp);
        agent->event = steal_pointer(&event);
        agent->systemd_dbus = steal_pointer(&bus);
        agent->interval_msec = 1000;
        agent->iteration = 0;
...
void cgtop_agent_unref(CGTopAgent *agent) {
        ...
        free_and_null(agent->root);
        sd_bus_unrefp(&agent->api_bus);
        sd_bus_unrefp(&agent->systemd_dbus);
        sd_bus_slot_unrefp(&agent->cgtop_slot);
        sd_event_unrefp(&agent->event);
}

static int cgtop_agent_reset_heartbeat_timer(CGTopAgent *agent, sd_event_source **event_source);

static int cgtop_agent_heartbeat_timer_callback(sd_event_source *event_source, uint64_t usec, void *userdata) {
        CGTopAgent *agent = (CGTopAgent *) userdata;

        int r = loop(agent);
        if (r < 0) {
                printf("Failed with loop: %s", strerror(-r));
                sd_event_exit(agent->event, r);
                return r;
        }

        r = cgtop_agent_reset_heartbeat_timer(agent, &event_source);
        if (r < 0) {
                printf("Failed to reset agent heartbeat timer: %s", strerror(-r));
                return r;
        }
...
static int event_reset_time_relative(
                sd_event *e,
                sd_event_source **s,
                clockid_t clock,
                uint64_t usec,
...
static int cgtop_agent_reset_heartbeat_timer(CGTopAgent *agent, sd_event_source **event_source) {
        return event_reset_time_relative(
                        ...
                        event_source,
                        CLOCK_BOOTTIME,
                        agent->interval_msec * USEC_PER_MSEC,
                        0,
                        cgtop_agent_heartbeat_timer_callback,
                        agent,
                        0,
...
int cgtop_agent_setup_timer(CGTopAgent *agent) {
        ...
        if (agent->interval_msec < 0) {
                printf("Interval %ld < 1, setting to 1", agent->interval_msec);
                return 0;
        }
